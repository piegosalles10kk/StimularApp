...
const CardAtividadeComProgresso = ({ 
  titulo, 
  descricao, 
  avatarUri, 
  progresso = 0, 
  ...
  return (
    ...
    >
      ...
      <VStack display='flex' flexDirection='column' flex={1} alignSelf='center'>
        <Titulo style={textStyle} fontWeight='bold' fontSize='lg' color='gray.600'>{titulo}</Titulo>
        <Text style={textStyle} fontSize='xs'>{descricao}</Text>
        {progresso > 0 && (
          <Progress.Bar
            progress={progresso / 100}
            width={null}
            color='#8cd114'
            style={{ marginTop: 10, marginBottom: 10, width: '90%', alignSelf: 'center' }}
          />
        )}
      </VStack>
      <Button
...
export default CardAtividadeComProgresso;
